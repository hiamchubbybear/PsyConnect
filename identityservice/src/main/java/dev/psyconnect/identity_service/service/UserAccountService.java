...
public class UserAccountService implements UserDetailsService, IUserAccountService {
    ...
    public UserAccountCreationResponse createAccount(UserAccountCreationRequest request, Provider provider) {
        ...
        Account account = Account.builder().username(request.getUsername()).isActivated(false).createdAt(Timestamp.from(Instant.now())).provider(provider).email(request.getEmail()).password(encodedPassword).isActivated((provider != Provider.ORDINARY)).role(roles).session(session).token(activateToken).profileId(profileId).build();
        ...
        try {
            ...
            kafkaService.sendLog(buildLog("identity-service", request.getUsername(), "Create account", e.getMessage(), Map.of("error", e.getMessage()), LogLevel.ERROR));
            log.info(e.getMessage());
            throw new CustomExceptionHandler(ErrorCode.UNCATEGORIZED_EXCEPTION);
        }
        String fullName = request.getFirstName() + " " + request.getLastName();
        if (provider == Provider.ORDINARY) {
            kafkaService.send(NOTIFICATION_CREATE_TOPIC, CreateAccountNotificationRequest.builder().code(activationCode).username(request.getUsername()).email(savedAccount.getEmail()).fullname(fullName).build());
        }
        kafkaService.sendLog(buildLog("identity-service", request.getUsername(), "Create account", "Success", Map.of("fullname", fullName, "email", savedAccount.getEmail()), LogLevel.LOG));

        return UserAccountCreationResponse.builder().username(request.getUsername()).email(account.getEmail()).imageUri(request.getAvatarUri()).role(roles).build();
    }
    ...
    public boolean deleteAccountWithoutCheck(UUID accountId) {
        try {
            ...
            kafkaService.sendLog(buildLog("identity-service", accountId.toString(), "Delete account (force)", "Success", Map.of("status", true), LogLevel.AUDIT));
            return true;
        } catch (Exception e) {
            kafkaService.sendLog(buildLog("identity-service", accountId.toString(), "Delete account (force)", "Failed", Map.of("error", e.getMessage()), LogLevel.ERROR));
            throw new CustomExceptionHandler(ErrorCode.UNCATEGORIZED_EXCEPTION);
        }
    }
    ...
    public DeleteAccountResponse deleteAccount(DeleteAccountRequest deleteAccountRequest, UUID uuid) {
        Account userObject = userAccountRepository.findById(uuid).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));

        if (!userObject.getUsername().equals(deleteAccountRequest.getUsername()) || PasswordEncodingService.getBCryptPasswordEncoder().matches(userObject.getPassword(), deleteAccountRequest.getPassword()))
        ...
        kafkaService.sendLog(buildLog("identity-service", userObject.getUsername(), "Delete account", "Requested", Map.of("email", deleteAccountRequest.getEmail()), LogLevel.LOG));

        return DeleteAccountResponse.builder().isSuccess(true).secret(SecretEnum.getRandomMessage()).session(userObject.getSession()).timestamp(deleteAccountRequest.getTimestamp()).build();
    }

    public boolean deleteAccountRequest(DeleteAccountConfirmRequest removeAccountRequest, UUID uuid) {
        Account userObject = userAccountRepository.findById(uuid).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));
        if (!removeAccountRequest.getSession().equals(userObject.getSession()) || !userObject.getToken().getToken().equals(removeAccountRequest.getToken()))
            throw new CustomExceptionHandler(ErrorCode.DELETE_ACCOUNT_FAILED);
        try {
            userAccountRepository.deleteById(uuid);
            kafkaService.sendLog(buildLog("identity-service", userObject.getUsername(), "Delete account", "Success", Map.of("metadata", true), LogLevel.LOG));
        } catch (Exception e) {
            kafkaService.sendLog(buildLog("identity-service", userObject.getUsername(), "Delete account", "Failed", Map.of("error", e.getMessage()), LogLevel.ERROR));
            throw new CustomExceptionHandler(ErrorCode.UNCATEGORIZED_EXCEPTION);
        }
        return true;
    }
    ...
    public UserInfoResponse getUserAccount(UUID accountId) {
        var userResponse = userAccountRepository.findById((accountId)).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));
        if (userAccountRepository.existsByUsernameAndIsActivatedTrue(userResponse.getUsername())) {
            kafkaService.sendLog(buildLog("identity-service", userResponse.getUsername(), "Get user account", "Success", Map.of("status", true), LogLevel.LOG));
            return UserInfoResponse.builder().username(userResponse.getUsername()).accountId(userResponse.getAccountId()).createdAt(userResponse.getCreatedAt().toLocalDateTime().toLocalDate()).role(userResponse.getRole()).email(userResponse.getEmail()).isActivated(userResponse.isActivated()).provider(userResponse.getProvider()).build();
    ...
    public AuthenticationFilterRequest loadUserByUsername(String username) throws CustomExceptionHandler {
        ...
        return AuthenticationFilterRequest.builder().username(foundObject.getUsername()).password(foundObject.getPassword()).build();
    }
    ...
    public UpdateAccountResponse updateAccount(UpdateAccountRequest updateAccountRequest, UUID accountId) {
        Account foundObject = userAccountRepository.findById(accountId).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));
        ...
        if (updated) {
            ...
            kafkaService.send("notification.account-change", UpdateAccountNotificatorRequest.builder().email(foundObject.getEmail()).username(foundObject.getUsername()).build());
            kafkaService.sendLog(buildLog("identity-service", foundObject.getUsername(), "Update account", "Success", Map.of("newEmail", foundObject.getEmail()), LogLevel.LOG));
        }
        return UpdateAccountResponse.builder().email(foundObject.getEmail()).username(foundObject.getUsername()).build();
    }
    ...
    public ActivateAccountResponse activateAccount(ActivateAccountRequest request) {
        ...
        var foundObject = activateRepository.findByToken(activateToken).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.ACTIVATION_FAILED));

        if (foundObject.getExpires().before(new Date())) {
            kafkaService.sendLog(buildLog("identity-service", request.getEmail(), "Activate account", "Failed", Map.of("reason", "Token expired"), LogLevel.WARN));

            return ActivateAccountResponse.builder().isSuccess(false).message("Activation token has expired").build();
        }
        ...
        return ActivateAccountResponse.builder().isSuccess(true).message("Account activation successful").build();
    }
    ...
    public Boolean requestActivateAccount(RequestActivationAccount requestActivationAccount) {
        if (accountRepository.existsByUsernameAndIsActivatedTrue(requestActivationAccount.getUsername()))
            throw new CustomExceptionHandler(ErrorCode.ACTIVATED);
        ActivateAccountNotificationRequest req = ActivateAccountNotificationRequest.builder().email(requestActivationAccount.getEmail()).fullname(requestActivationAccount.getFullname()).code(generateActivationCode()).username(requestActivationAccount.getUsername()).build();
        kafkaService.send((NOTIFICATION_CREATE_TOPIC), req);
        sendNotification(ActivateAccountNotificationRequest.builder().username(requestActivationAccount.getUsername()).email(requestActivationAccount.getEmail()).build(), Provider.ORDINARY);
        kafkaService.sendLog(buildLog("identity-service", requestActivationAccount.getUsername(), "Request account activation", "Success", Map.of("metadata", req), LogLevel.LOG));
        return true;
    }

    public Boolean requestPasswordReset(RequestPasswordReset requestActivationAccount) {
        var userFound = accountRepository.findByEmail(requestActivationAccount.getEmail()).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));
        ActivateAccountNotificationRequest req = ActivateAccountNotificationRequest.builder().email(userFound.getEmail()).fullname(requestActivationAccount.getFullName()).code(generateActivationCode()).username(userFound.getUsername()).build();
        log.info("Request found {}", req);
        kafkaService.send((NOTIFICATION_RESET_TOPIC), req);
        sendNotification(ActivateAccountNotificationRequest.builder().username(userFound.getUsername()).email(requestActivationAccount.getEmail()).code(req.getCode()).build(), Provider.ORDINARY);
        kafkaService.sendLog(buildLog("identity-service", userFound.getUsername(), "Request account password reset", "Success", Map.of("metadata", req), LogLevel.LOG));
        return true;
    }

    private LogEvent buildLog(String service, String userId, String action, String message, Map<String, ?> metadata, LogLevel level) {
        return LogEvent.builder().service(service).level(level).timestamp(Instant.now().toString()).userId(userId).action(action).message(message).metadata(metadata).build();
    }
    ...
    public Token saveActivationModel(String username, String activationCode, Provider provider) {
        ...
        Token token = tokenRepository.findById(username).orElse(Token.builder().token(activationCode).username(username).issuedAt(Timestamp.from(Instant.now())).provider(provider.toString()).revoked(false).build());
    ...
    public Boolean resetPassword(PasswordResetRequest passwordResetRequest) {
        var userFound = userAccountRepository.findByEmail(passwordResetRequest.getEmail()).orElseThrow(() -> new CustomExceptionHandler(ErrorCode.USER_NOT_FOUND));
