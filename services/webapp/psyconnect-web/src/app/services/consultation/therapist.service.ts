...
export class TherapistService {
  private readonly baseUrl = `${environment.apiUrl}/${environment.apiVersion}/consultation/therapist`;

  constructor(private http: HttpClient) {}

  getTherapistProfile(): Observable<TherapistV1> {
    return this.http.get<TherapistV1>(this.baseUrl);
  }
  ...
  createTherapistProfile(data: TherapistV1): Observable<TherapistV1> {
    return this.http.post<TherapistV1>(this.baseUrl, data);
  }

  updateTherapistProfile(data: Partial<TherapistV1>): Observable<TherapistV1> {
    return this.http.put<TherapistV1>(this.baseUrl, data);
  }

  changeTherapistStatus(status: boolean): Observable<boolean> {
    return this.http.put<boolean>(`${this.baseUrl}/status/${status}`, {});
  }
}
